...
#' 
#' @title Fit different models to single cohort element data
#' 
#' @param  mass.remaining proportional mass loss, that is, mi/m0
#' 
#' @param  element.remaining proportional element remaining ni/n0
#'
#' @param model there is one model currently implemented (see below)
#' 
#' @param iters Number of random starts for the fitting.  Use higher numbers for models with larger numbers of parameters and for models that inherently tend to be less identifiable.
#' 
#' @param upper,lower \bold{Optional} user specified values for the upper and lower bounds used by
...
#' @param ...  Additional arguments passed to \code{\link{optim}}
#' 
#' @references \itemize{ \item{Parton, W., et al. (2007) Global-scale similarities in nitrogen release patterns during long-term decomposition. Science 315.5810: 361-364.}}
#' 
#' @return returns a elementfit object with the following elements: 
#'  \itemize{
#' \item{\bold{optimFit:} a list generated by the \code{\link{optim}} function}
#' \item{\bold{logLik:} the log-likelihood of the model}
#' \item{\bold{element:} vector of element (same as input)}
#' \item{\bold{mass:} vector os mass remaining (same as input)}
#' \item{\bold{predicted:} predicted values from the model for each of the points within time}
#' \item{\bold{model:} name of the model }
#' \item{\bold{nparams:} number of fit parameters in the model}
...
fit_element <- function(mass.remaining, element.remaining, model = c("import.model"), iters = 500, 
                       upper = NULL, lower = NULL, ...) {
  
  if (length(element.remaining) != length(mass.remaining)) {
    stop("Element remaining vector must have the same length and correspond to the mass remaining vector")
  }
  if (min(mass.remaining) > 2 | max(mass.remaining) > 200) {
    stop("Check mass remaining vector; must be in proportional mass remaining.")
  }
  
  if (min(element.remaining) < 0) {
    stop("Check element.remaining vector; negative values detected.")
  }
  ...
  fit <- multioptimFit(mass.remaining, element.remaining, model, iters = iters, upper = upper, 
                       lower = lower, ...)
  if (is.null(fit)) 
    return(NULL)
  predicted_vals <- do.call(model, c(list(mass.remaining), as.list(fit$par)))
  LL <- calculateLL(predicted_vals, element.remaining, sqrt(fit$value/length(mass.remaining)))
  nps <- length(fit$par)
  model.AIC <- calculateAIC(LL, nps)
  model.AICc <- calculateAICc(LL, nps, length(mass.remaining))
  model.BIC <- calculateBIC(LL, nps, length(mass.remaining))
  fit.out <- list(optimFit = fit, logLik = LL, fitAIC = model.AIC, fitAICc = model.AICc, 
                  fitBIC = model.BIC, mass = mass.remaining, element.remaining=element.remaining,predicted = predicted_vals, 
                  model = model, nparams = nps)
  class(fit.out) <- "elementfit"
