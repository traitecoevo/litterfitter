...
discrete.series <- function(x, R, K1, K2, upper = c((1 - 1e-04), 1000, 1000), lower = c(1e-04, 1e-04, 1e-04)) {
...
multioptimFit <- function(time_data, mass_data, model, iters = 200, upper = NULL, lower = NULL, ...) {
  ...
  # need this to allow variation in number of parameters (changed to 3 because 'upper' and 'lower' are now formal
  # arguments)
  fit <- list()
