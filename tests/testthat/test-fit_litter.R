...
test_that("plots dont throw errors", {
  ...
  expect_that(plot_multiple_fits(time = pineneedles$Year, mass.remaining = pineneedles$Mass.remaining,model=c("neg.exp", "weibull"),
                     bty = 'n', iters = 2000), not(gives_warning()))

})

test_that("crazy input throws errors",{
  expect_error(fit_litter(time = c(0, 1, 2, 3, 4, 5, 6), mass.remaining = c(1000, 0.9, 1.01, 0.4, 0.6, 0.2, 0.01),
             model = "weibull", iters = 10))
  expect_error(fit_litter(time = c(0, -1, 2, 3, 4, 5, 6), mass.remaining = c(1, 0.9, 1.01, 0.4, 0.6, 0.2, 0.01),
                          model = "weibull", iters = 10))
})
