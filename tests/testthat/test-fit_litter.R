...
test_that("plots and summaries dont throw errors", {
  ...
  expect_output(plot_multiple_fits(time = pineneedles$Year, mass.remaining = pineneedles$Mass.remaining, model = c("neg.exp", "weibull"), bty = "n", 
                                   iters = 2000),"Number of successful fits")
...
test_that("bootstraps are behaving", {
  fit <- fit_litter(time = c(0, 1, 2, 3, 4, 5, 6), mass.remaining = c(1, 0.9, 1.01, 0.4, 0.6, 0.2, 0.01), model = "neg.exp", iters = 2000)
